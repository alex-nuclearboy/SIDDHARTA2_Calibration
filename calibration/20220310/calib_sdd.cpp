...
void calib_sdd() {
  ...
  fitFuncTotal = new TF1(Form("fitFuncTotal_%d_%d",busNumber,sddNumber),fitTotalFunc,1500,4500,36);
  ...
  for (Int_t k = 0; k < 6; k++) {
    fitFuncTotal->SetParameter(k,fitFuncGaussTailTiKalphaStart->GetParameter(k));
  }

  for(Int_t k = 6; k < 12; k++) {
    fitFuncTotal->SetParameter(k,fitFuncGaussTailTiKbetaStart->GetParameter(k-6));
  }

  for (Int_t k = 12; k < 16; k++) {
    fitFuncTotal->SetParameter(k,fitFuncGaussMnKalphaStart->GetParameter(k-12));
  }

  for (Int_t k = 16; k < 20; k++) {
    fitFuncTotal->SetParameter(k,fitFuncGaussFeKalphaStart->GetParameter(k-16));
  }

  for (Int_t k = 20; k < 26; k++) {
    fitFuncTotal->SetParameter(k,fitFuncGaussTailCuKalphaStart->GetParameter(k-20));
  }

  for(Int_t k = 26; k < 32; k++) {
    fitFuncTotal->SetParameter(k,fitFuncGaussTailCuKbetaStart->GetParameter(k-26));
  }

  for(Int_t k = 32; k < 36; k++) {
    fitFuncTotal->SetParameter(k,fitFuncBkgrndStart->GetParameter(k-32));
  }

  hSDD_fit->Fit(fitFuncTotal,"","",1500,4000);
  ...
  for(int l=0; l<4; l++) {
    fitFuncGaussCuKalpha->SetParameter(l,fitFuncTotal->GetParameter(l+20));
  }

  fitFuncTailCuKalpha = new TF1(Form("fitFuncTailCuKalpha_%d_%d",busNumber,sddNumber),fitTailFunc,1500,4500,5);
  fitFuncTailCuKalpha->SetParameter(0,(fitFuncTotal->GetParameter(20)*fitFuncTotal->GetParameter(24)));
  fitFuncTailCuKalpha->SetParameter(1,fitFuncTotal->GetParameter(21));
  fitFuncTailCuKalpha->SetParameter(2,fitFuncTotal->GetParameter(22));
  fitFuncTailCuKalpha->SetParameter(3,fitFuncTotal->GetParameter(23));
  fitFuncTailCuKalpha->SetParameter(4,fitFuncTotal->GetParameter(25));

  fitFuncGaussCuKbeta = new TF1(Form("fitFuncGaussCuKbeta_%d_%d",busNumber,sddNumber),fitGaussFunc,3300,3500,4);
  for(int l=0; l<4; l++) {
    fitFuncGaussCuKbeta->SetParameter(l,fitFuncTotal->GetParameter(l+26));
  }

  fitFuncTailCuKbeta = new TF1(Form("fitFuncTailCuKbeta_%d_%d",busNumber,sddNumber),fitTailFunc,1500,4500,5);
  fitFuncTailCuKbeta->SetParameter(0,(fitFuncTotal->GetParameter(26)*fitFuncTotal->GetParameter(30)));
  fitFuncTailCuKbeta->SetParameter(1,fitFuncTotal->GetParameter(27));
  fitFuncTailCuKbeta->SetParameter(2,fitFuncTotal->GetParameter(28));
  fitFuncTailCuKbeta->SetParameter(3,fitFuncTotal->GetParameter(29));
  fitFuncTailCuKbeta->SetParameter(4,fitFuncTotal->GetParameter(31));

  fitFuncBkgrnd = new TF1(Form("fitFuncBkgrnd_%d_%d",busNumber,sddNumber),fitBkgndFunc,1500,4500,4);
  for(int l=0; l<4; l++) {
    fitFuncBkgrnd->SetParameter(l,fitFuncTotal->GetParameter(l+32));
  }
